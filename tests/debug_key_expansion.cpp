...
int main() {
    ...
    // Compare each generated round key with the expected round keys
    for (vector<vector<uint8_t>>::size_type i = 0; i < expectedRoundKeys.size(); ++i) {
        if (roundKeys[i] != expectedRoundKeys[i]) {
            cout << "Round Key " << i << red << " FAILED" << reset << endl;
            cout << "Expected: ";
            for (const auto& byte : expectedRoundKeys[i]) {
                cout << hex << setw(2) << setfill('0') << (int)byte << " ";
            }
            cout << endl;
            cout << "Got:      ";
            for (const auto& byte : roundKeys[i]) {
                cout << hex << setw(2) << setfill('0') << (int)byte << " ";
            }
            cout << endl;
        } else {
            cout << "Round Key " << i << green << " PASSED" << reset << endl;
        }
    }
